...
class JangKeyteCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (!Schema::hasTable($this->tableName)) {
            ...
            if (!Schema::hasTable($this->tableName)) {
                ...
                if($this->tableName == '') { $this->tableName .= 's'; }
            }
        }
    ...
    // Start Filter Codes==========================================================================
    public function filterCodes($model)
    {
        $modelName = ucfirst($model);

        // Create the directory if it doesn't exist
        $directoryPath = base_path("modules/{$this->moduleName}/src/Models");
        ...
        foreach($this->columns as $column => $type) { 
            $fillable .= ($fillable == '') ? "'" . $column . "'" : "," . "'" . $column . "'";
        }

        JangKeyteStub::from( $this->stubPath . 'model.stub')
                     ->to($directoryPath)
                     ->name($modelName)
                     ->ext('php')
                     ->replaces([
                        'NAMESPACE' => 'Modules\\' . $this->moduleName . '\src\Models',
                        'CLASS' => $modelName,
                        'FILLABLE' => '[' . $fillable . ']'
                    ])->generate();
