        ...
        <div class="col-md-12">
                ...
                @foreach($datas as $item)
                    <div class="row border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                        <div class="col-auto d-none d-lg-block py-2">
                            <img src="{{ asset('storage/uploads/activity/' . ($item->photo ?? '../default.png')) }}" width="200" max-height="250">
                        </div>
                        <div class="col ps-4 pb-3 d-flex flex-column position-static">
                            ...
                            <a href="{{ route('hoat_dong_chi_tiet', $item->slug) }}" class="icon-link gap-1 icon-link-hover stretched-link text-decoration-none">
                                {{ __('Continue reading') }}
                                <svg class="bi">
            ...
            @else
                <div class="alert alert-danger" role="alert">
                    {{ __('Hiện tại chưa có hoạt động nào!!!') }}
                </div>
            @endif
        </div>
