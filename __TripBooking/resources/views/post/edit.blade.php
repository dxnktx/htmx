...
@section('button')
<a href="{{ route('post_index') }}" class="btn btn-primary btn-sm ms-2"><i class="bi bi-folder-check"></i> {{ __('View All') }}</a>
@endsection
                    ...
                    <form method="POST" action="{{ route('post_update') }}" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="alert alert-primary d-flex align-items-center m-2" role="alert">
                                <div>
                                    <h5>{{ __('Update post') }}</h5>
                                    <span>{{ __('You can make any changes on the form below and click [Save] button to update post information.') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class='row mb-3'>
                            ...
                            <div class='col-sm-10'>
                                <input type='text' name='heading' class='form-control @error('heading') is-invalid @enderror' value='{{ old('heading', $item->heading) }}' placeholder="{{ __('Nhập tiêu đề của bài viết') }}"/>
                        ...
                        <div class='row mb-3'>
                            ...
                            <div class='col-sm-10'>
                                <input type='text' name='slug' class='form-control @error('slug') is-invalid @enderror' value='{{ old('slug', $item->slug) }}' placeholder="{{ __('Nếu để trống hệ thống sẽ tự động lấy theo tiêu đề') }}"/>
                        ...
                        <div class='row mb-3'>
                            ...
                            <div class='col-sm-10'>
                                <input type='text' name='total_view' class='form-control @error('total_view') is-invalid @enderror' value='{{ old('total_view', $item->total_view ?? 0) }}' placeholder="{{ __('Mặc định là 0') }}"/>
                        ...
                        <div class='row mb-3'>
                            ...
                            <div class='col-sm-10'>
                                <img src="{{ asset('storage/uploads/post/' . ($item->photo ?? '../default.png')) }}" height="50px">
                            </div>
                        </div>
